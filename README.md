...
生产更新 `api`、`identity` 或前端路由后，可以使用统一回归脚本检查主域路径边界、登录续签、JWT `iss/aud/kid` 元数据、刷新会话查询、云盘聚合资料、存储概览、管理员云盘用户入口、Identity 审计日志查询、Identity Flyway 迁移历史、旧身份路径 404、注销失效和审计日志最新行：
