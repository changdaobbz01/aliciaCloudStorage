...
`identityApi` is the owner of Alicia's shared identity boundary.
...
- It owns application-level role assignments; current cloud-drive administrator authorization is represented as `appRoles.cloud=CLOUD_ADMIN`, RAG roles are represented as `appRoles.rag=RAG_*`, and global `ADMIN` remains an identity super-admin fallback.
...
Application roles are stored in `identity_user_app_role` and returned in `IdentityUserResponse.appRoles`. Role codes are currently constrained to `<APP>_USER` and `<APP>_ADMIN`; app codes are lowercase identifiers such as `cloud` and `rag`. Existing global administrators were seeded as `cloud/CLOUD_ADMIN` when the table was introduced. Effective roles now include `cloud/CLOUD_USER` and `rag/RAG_USER` for regular users, while global `ADMIN` users always remain effective `cloud/CLOUD_ADMIN` and `rag/RAG_ADMIN` even if an explicit lower known-application role is present.

`CloudStorageApi` consumes `appRoles.cloud` for cloud-drive administrator authorization, and RAG consumes `appRoles.rag` before assistant planning endpoints execute.
...
Set `ALICIA_IDENTITY_MYSQL_DATABASE` to keep this service on a dedicated MySQL database on the same MySQL server; `.env.example` defaults to `alicia_identity`, and the MySQL compose service creates it on first volume initialization through `deploy/mysql/init-identity-database.sh`. Production has completed the identity database split and cloud identity residue cleanup. Older shared-database environments can still use `deploy/scripts/apply-identity-database-split.sh` to copy identity-owned tables and `identity_flyway_schema_history` to the target database, update `.env`, restart identity, and run the shared route verification; after the split is verified, use `deploy/scripts/drop-cloud-identity-residue.sh` to back up and remove the old identity-owned tables from the cloud database.

Use `deploy/scripts/generate-identity-rs256-env.sh` to generate an RS256 key pair and a git-ignored `.env` snippet. When switching a deployed environment from HS256 to RS256, keep the previous HS256 signing secret in `ALICIA_AUTH_TOKEN_PREVIOUS_KEYS` until older access tokens expire; if `.env` omits `ALICIA_AUTH_TOKEN_KEY_ID`, the helper uses the compose default `alicia-hs256-v1`. After production is already on RS256, use the same generator for the next RSA key pair, then use `deploy/scripts/prepare-identity-rs256-rotation-env.sh` to create a candidate `.env` that promotes the new key and keeps the current RSA public key in `ALICIA_AUTH_TOKEN_PREVIOUS_RSA_PUBLIC_KEYS`.
...
When a historical RS256 JWT key is no longer needed, use `deploy/scripts/prepare-identity-rs256-previous-key-removal-env.sh <old-rs256-kid>` to remove it from `ALICIA_AUTH_TOKEN_PREVIOUS_RSA_PUBLIC_KEYS`. When a historical HS256 JWT key is no longer needed, use `deploy/scripts/prepare-identity-hs256-key-removal-env.sh` to create a candidate `.env` that removes it from `ALICIA_AUTH_TOKEN_PREVIOUS_KEYS`; it defaults to `alicia-hs256-v1` and does not overwrite production `.env`. In unlaunched environments, `deploy/scripts/apply-identity-hs256-key-removal-env.sh` can perform the backup, apply, identity restart, verification, and automatic rollback as one operation.
