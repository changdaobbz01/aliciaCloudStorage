...
When a historical HS256 JWT key is no longer needed, use `deploy/scripts/prepare-identity-hs256-key-removal-env.sh` to create a candidate `.env` that removes it from `ALICIA_AUTH_TOKEN_PREVIOUS_KEYS`; it defaults to `alicia-hs256-v1` and does not overwrite production `.env`.
