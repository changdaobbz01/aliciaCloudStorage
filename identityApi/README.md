...
Set `ALICIA_IDENTITY_MYSQL_DATABASE` to move this service to a dedicated MySQL database on the same MySQL server; when it is unset, compose keeps using `MYSQL_DATABASE`. Use `deploy/scripts/apply-identity-database-split.sh` to copy identity-owned tables and `identity_flyway_schema_history` to the target database, update `.env`, restart identity, and run the shared route verification. After the split is verified, use `deploy/scripts/drop-cloud-identity-residue.sh` to back up and remove the old identity-owned tables from the cloud database.
