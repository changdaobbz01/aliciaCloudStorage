...
OUTPUT_DIR="${ALICIA_HS256_CLEANUP_OUTPUT_DIR:-deploy/generated/identity-rs256}"
...
filter_previous_keys() {
    ...
    for entry in "${entries[@]}"; do
        ...
        [[ "$trimmed_entry" == *"="* ]] || fail "Malformed ALICIA_AUTH_TOKEN_PREVIOUS_KEYS entry without '='."

        entry_key_id="$(trim "${trimmed_entry%%=*}")"
        [[ -n "$entry_key_id" ]] || fail "Malformed ALICIA_AUTH_TOKEN_PREVIOUS_KEYS entry with empty key id."
...
PREVIOUS_KEYS="$(dotenv_value "$ENV_FILE" ALICIA_AUTH_TOKEN_PREVIOUS_KEYS)"
...
[[ "$TOKEN_ALGORITHM" == "RS256" ]] || fail "Current env must sign with RS256 before removing historical HS256 keys."
...
[[ -n "$PREVIOUS_KEYS" ]] || fail "Base env has no ALICIA_AUTH_TOKEN_PREVIOUS_KEYS to clean."
[[ "$TOKEN_EXPIRE_SECONDS" =~ ^[0-9]+$ ]] || fail "ALICIA_AUTH_TOKEN_EXPIRE_SECONDS must be a number when set."

filter_previous_keys "$PREVIOUS_KEYS" "$TARGET_KEY_ID"
[[ "$REMOVED_COUNT" -gt 0 ]] || fail "Key id $TARGET_KEY_ID was not found in ALICIA_AUTH_TOKEN_PREVIOUS_KEYS."

umask 077
mkdir -p "$OUTPUT_DIR"

TIMESTAMP="$(date -u +%Y%m%d%H%M%S)"
CANDIDATE_FILE="$OUTPUT_DIR/remove-$TARGET_KEY_ID.candidate.$TIMESTAMP.env"
BACKUP_FILE="$OUTPUT_DIR/remove-$TARGET_KEY_ID.backup.$TIMESTAMP.env"
TEMP_FILE="$CANDIDATE_FILE.tmp"

cp "$ENV_FILE" "$TEMP_FILE"
replace_or_append_env "$TEMP_FILE" "$CANDIDATE_FILE" ALICIA_AUTH_TOKEN_PREVIOUS_KEYS "$FILTERED_PREVIOUS_KEYS"
rm -f "$TEMP_FILE"
chmod 600 "$CANDIDATE_FILE"

printf 'Prepared HS256 previous-key removal candidate env:\n'
printf '  base env:           %s\n' "$ENV_FILE"
printf '  candidate env:      %s\n' "$CANDIDATE_FILE"
printf '  backup path:        %s\n' "$BACKUP_FILE"
printf '  current alg/kid:    %s/%s\n' "$TOKEN_ALGORITHM" "$CURRENT_KEY_ID"
printf '  removed HS256 kid:  %s\n' "$TARGET_KEY_ID"
