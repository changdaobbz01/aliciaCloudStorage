...
Invoke-Step "verify cloud frontend split wiring" {
    ...
    Require-Contains "webApp/scripts/verify-session-sync.mjs" "cloud web profile session revocation must preserve the current revoked filter" "cloud web profile session revocation must preserve the selected revoked filter"
    ...
    Require-Contains "sysManage/src/components/AppPackagePanel.tsx" "okButtonProps={{ danger: true, loading: deleting, disabled: uploading || deleting }}" "cloud console APK delete confirmation must surface pending delete state"
