...
Invoke-Step "verify cloud frontend split wiring" {
    ...
    Require-Contains "webApp/package.json" "vite build && npm run verify:bundle-size" "cloud web build must run the bundle size verifier after vite build"
    ...
    Require-Contains "sysManage/package.json" "vite build && npm run verify:bundle-size" "cloud console build must run the bundle size verifier after vite build"
